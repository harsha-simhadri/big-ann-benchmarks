...
class Zilliz(BaseOODANN):
    ...
    def translate_dist_fn_searcher(self, metric):
        if metric == 'euclidean':
            return 'L2'
        elif metric == 'ip':
            return 'IP'
    ...
    def fit(self, dataset):
        ...
        gc.collect()
        g = ood_searcher.Graph(graph_path, 'diskann')
        self.searcher = ood_searcher.Searcher(
            g, ds.get_dataset(), self.translate_dist_fn_searcher(ds.distance()), "SQ8U")
    ...
    def __str__(self):
        return f'zilliz_({self.index_name()})_ef{self.ef}'
