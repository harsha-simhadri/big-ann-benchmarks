...
class Zilliz(BaseANN):
    ...
    def fit(self, dataset): # e.g. dataset = "sparse-small"
        ...
        print("start add")
        path = f'zilliz_R{self.R}_L{self.L}.idx'

        self.searcher = sparse_searcher.SparseGrapSearcher(self.ds.get_dataset_fn(), path, self.R, self.L)
        print("done add")
    ...
    def __str__(self):
        return f'zilliz_qdrop{self._budget}_ef{self.ef}'
