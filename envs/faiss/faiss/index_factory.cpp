...
char get_trains_alone(const Index* coarse_quantizer) {
    ...
    if (dynamic_cast<const IndexHNSWFlat*>(coarse_quantizer) ||
        dynamic_cast<const IndexHNSWlib*>(coarse_quantizer)) {
        return 2;
    }
    ...
    for (char* tok = strtok_r(&description[0], " ,", &ptr); tok;
        ...
        if (sscanf(tok, "PCA%d", &d_out) == 1) {
            ...
            coarse_quantizer_1 = new IndexHNSWFlat(d, M, metric);
                ...
                !coarse_quantizer &&
                sscanf (tok, "IVF%" PRId64 "_HNSWlibFp32_%d",&ncentroids, &M) == 2) {
            coarse_quantizer_1 = new IndexHNSWlibFp32 (d, M, metric);

        } else if (
                !coarse_quantizer &&
                sscanf (tok,"IVF%" PRId64 "_HNSWlibBfp16_%d", &ncentroids, &M) == 2) {
            coarse_quantizer_1 = new IndexHNSWlibBfp16 (d, M, metric);

        } else if (
                !coarse_quantizer &&
                sscanf (tok,"IVF%" PRId64 "_HNSWlibInt16_%d", &ncentroids, &M) == 2) {
            coarse_quantizer_1 = new IndexHNSWlibInt16 (d, M, metric);

        } else if (
                !coarse_quantizer &&
                sscanf (tok,"IVF%" PRId64 "_HNSWlibInt8_%d", &ncentroids, &M) == 2) {
            coarse_quantizer_1 = new IndexHNSWlibInt8 (d, M, metric);
