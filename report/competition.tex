...
system makes it difficult to go beyond this scale.  Implementing most
...
billion+ scale, this competition aims rigorously benchmark the
...
index with minimal hardware cost.  Specifically, this scale a good fit
...
these diverse use-cases, the competition will run evaluations on
datasets representing these applications.


To provide a platform fot the development of algorithmic and systems innovation,
the challenge provides both  standardized and the custom hardware tracks.

Since most current solutions are limited in scale as they require data
and indices to be stored in expensive main memory, we limit and
normalize the amount of DRAM available in standardized hardware track
...
Additionally, we allow for a second standardized hardware (T2) where
in an additional 1TB of inexpensive SSD is allowed to serve
...
In specialized/custom hardware track T3, we encourge the use of any
...
are at least 100 dimensional and use either an {\tt int8} or a {\tt
  float32} representation of data so that the dataset is 100s of GB in
