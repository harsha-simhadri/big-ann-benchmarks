...
\title[Big-ANNS results]{Results of the Billion-Scale Approximate Nearest Neighbor Search Challenge}
...
\author{%
 ...
 \Name{Jingdong Wang\aff{1}} \Email{jingdw@microsoft.com}\\
  \rm \small \\
...
\begin{abstract}
  ...
  up an open source evaluation framework\footnote{\url{https://github.com/harsha-simhadri/big-ann-benchmarks/}}%~\citep{framework} 
  and
  ...
  GigaBytes in size.  It uses FAISS~\citep{Faiss17} as the baseline.
  The standard hardware track T2 additional allows inexpensive SSDs in
  addition to the limited DRAM and uses DiskANN~\citep{DiskANN19} as
...
\matthijs{Citations should be with citep by default, cite does not add parentheses. Reference URLs as footnotes, otherwise it looks ugly.} 
