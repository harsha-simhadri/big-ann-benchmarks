...
def init_data(args):
    ...
    data_dir = "data/run.%s.gpu=%d.time=%f" % ( os.path.basename(args.indexfile), args.quantizer_on_gpu_search, now )
    os.mkdir( data_dir )

    # initialize the main benchmarks file
    data_file = os.path.join(data_dir,"data.txt")
    f = open( data_file, "w+")
    f.close()

# Write the benchmarks
def write_data( inter_measure, ms_per_query, min_time, tot_queries, power_stats):

    # get a timestamp
    now = time.time()

    # write the power stats to the timestamp file as JSON
    f = open( data_dir+ "/%f" % now, "a+")
    f.write( json.dumps( power_stats ) )
    f.flush()
    f.close()

    # extract the total power from power stats
    power_cons = -1
    if power_stats!={}:
        power_cons = power_stats["tot_power"]

    # write the main benchmarks to the main data file
    f = open( os.path.join(data_dir, "data.txt"), "a+")
    f.write("%f, %f, %f, %f, %f, %f, %f\n" %(  now, inter_measure, ms_per_query, min_time, power_cons, tot_queries, power_cons/tot_queries ) )
    f.flush()
    f.close()
...
def eval_setting(args, index, xq, gt, k, inter, min_time, rgsearch=False):
    ...
    # Start power capture
    if args.search:
        capid = power_capture.start()

    t0 = time.time()
    while True:
        if rgsearch: # Do range search
            nres, D, I = index.range_search(xq, rgsearch)
    ...
    if rgsearch:
        print("Sample of range search results", nres[0], nres[1], I[nres[0]:nres[1]])

    # Stop power capture
    power_stats = {}
    if args.search:
        power_stats = power_capture.stop(capid,all_stats=True)

    ms_per_query = ((t1 - t0) * 1000.0 / nq / nrun)

    if rgsearch: # Range search computes auc
        from eval_range import compute_AP
        auc = compute_AP( gt, (nres, I, D ) )
    elif inter: # Compute recall@k
        rank = k
    ...
    if args.search:
        # Capture/report the benchmarks 
        if not rgsearch:
            write_data( inter_measure, ms_per_query, min_time,  nq*nrun, power_stats)
        else:
            print("Got range search auc=", auc)
...
def run_experiments_searchparams(ds, index, args, rgsearch=False):
    k = args.k

    xq = ds.get_queries()
    if rgsearch: # range result returns different results
        nres, gt_I, gt_D = ds.get_groundtruth(k=k)
    else:
        gt_I, gt_D = ds.get_groundtruth(k=k)

    if rgsearch:
        gt = ( nres, gt_I, gt_D ) # computation of auc needs all of it
    else:
        gt = gt_I # computation of recall@10 only needs indices
    ...
    for params in searchparams:
        ...
        sys.stdout.flush()
        eval_setting(args, index_wrap, xq, gt, args.k, args.inter, args.min_test_duration, rgsearch )
...
def main():
    ...
    #GW ds = DATASETS[args.dataset]()
    ds = DATASETS[args.dataset]

    if args.search:
        # Do a sanity check before loading a large index file
        q = ds.get_queries()
        #range search- r, i, g = ds.get_groundtruth()
        i, g = ds.get_groundtruth()

    if args.build:
        # Do a sanity check before training large dataset
        d = ds.get_dataset()
        for xblock in ds.get_dataset_iterator(bs=10):
            break

    if args.search:

        # Connect to ipmicap server and do a test
        power_capture("192.168.99.123:1234:-1")
        power_capture.ping()
        capid = power_capture.start()
        time.sleep(2)
        power_cons = power_capture.stop(capid, all_stats=True)
        if power_cons==-1: raise Exception("Power capture failed.")
        print("Power sensor test passed=", power_cons)
    ...
    if args.search:
        ...
        if args.searchparams == ["autotune"]:
            ...
            run_experiments_searchparams(ds, index, args ) #, 96237 )
