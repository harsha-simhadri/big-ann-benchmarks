# T3 Track Leaderboards 

## Final Rankings On Private Query Set

*Available December 2021*
 
## Rankings On Public Query Set

### Rankings By Submission (alphabetical)

|Submission                                                 |Team                   |Hardware    |Status|R@10 Rank|Q/Sec Rank|KWh/Q Rank|$$ Rank| 
|-----------------------------------------------------------|-----------------------|------------|------|---------|----------|----------|-------|
|DeepGram                                                   |DeepGram               |NVidia GPU  |     -|        -|         -|         -|      -|
|DiskANN                                                    |Microsoft (org)        |Intel Optane|     -|        -|         -|         -|      -|
|[FAISS Baseline](/../../../blob/main/t3/faiss_t3/README.md)|Facebook Research (org)|NVidia GPU  | final|        1|         1|         1|      1|
|[Gemini](/../../../blob/gw/t3_gemini/t3/gemini/README.md)  |GSI Technology (org)   |Leda-E APU  |inprog|        -|         -|         -|      -|
|KANNDI                                                     |Silo.ai                |Leda-E APU  |     -|        -|         -|         -|      -|
|NVidia 1                                                   |NVidia                 |NVidia GPU  |     -|        -|         -|         -|      -|
|NVidia 2                                                   |NVidia                 |NVidia GPU  |     -|        -|         -|         -|      -|
|Optane 1                                                   |Intel                  |Intel Optane|     -|        -|         -|         -|      -|
|Optane 2                                                   |Intel                  |Intel Optane|     -|        -|         -|         -|      -|
|Vector T3                                                  |Vector Institute       |NVidia GPU  |     _|        -|         -|         -|      -|

*org*= submission by challenge organizer<br>
*final*=final submission<br>
*inprog*=algorithm development still in progress<br>

### Rankings Per Metric

#### Recall Rankings ( Recall@10 ) 

| Rank | Submission | Team              | Hardware        | Score |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     0 |

#### Throughput Rankings ( Queries/Second )

| Rank | Submission | Team              | Hardware        | Score |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     0 |

#### Power Rankings  ( KWh/Query )

| Rank | Submission | Team              | Hardware        | Score |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     0 |

#### Cost Rankings ( $$ )

| Rank | Submission | Team              | Hardware        | Score |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     0 |

### Rankings Per Database

#### Deep1B

##### Deep1B Recall Rankings 

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    - |

##### Deep1B Throughput Rankings 

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### Deep1B Power Rankings 

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### Deep1B Cost Rankings 

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  - |

#### BigANN

##### BigANN Recall Rankings

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    - |

##### BigANN Throughput Rankings

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### BigANN Power Rankings

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### BigANN Cost Rankings

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  - |

#### MSTuring

##### MSTuring Recall Rankings

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    - |

##### MSTuring Throughput Rankings

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### MSTuring Power Rankings

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### MSTuring Cost Rankings

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  - |

#### MSSpace

##### MSSpace Recall Rankings

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    0 |

##### MSSpace Throughput Rankings

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### MSSpace Power Rankings

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### MSSpace Cost Rankings

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  - |

#### Text-2-Image

##### Text-2-Image Recall Rankings

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    - |

##### Text-2-Image Throughput Rankings

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### Text-2-Image Power Rankings

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### Text-2-Image Cost Rankings

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  - |

#### FB-SimSearchNet

##### FB-SimSearchNet Recall Rankings

| Rank | Submission | Team              | Hardware        | R@10 |
|------|------------|-------------------|-----------------|------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |    - |

##### FB-SimSearchNet Throughput Rankings

| Rank | Submission | Team              | Hardware        | Q/Sec |
|------|------------|-------------------|-----------------|-------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |     - |

##### FB-SimSearchNet Power Rankings

| Rank | Submission | Team              | Hardware        | KWh/Q |
|------|------------|-------------------|-----------------|------------|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |          - |

##### FB-SimSearchNet Cost Rankings

| Rank | Submission | Team              | Hardware        | $$ |
|------|------------|-------------------|-----------------|----|
|    1 | FAISS T3   | Facebook Research | NVidia GPU      |  0 |

