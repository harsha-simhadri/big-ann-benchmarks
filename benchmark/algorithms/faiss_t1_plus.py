...
class Faiss(BaseANN):
    ...
    def refine_index_name(self, name):
        return f"data/refine_{name}.{self.indexkey}.faissindex"
    ...
    def fit(self, dataset):
        ...
        efSearch = index_params.get("quantizer_add_efSearch", 64)
        efConstruction = index_params.get("quantizer_efConstruction", 168)
        use_two_level_clustering = index_params.get("two_level_clustering", True)
        indexfile = self.refine_index_name(dataset)
        ...
        if index_ivf:
            ...
            if isinstance(quantizer, faiss.IndexRefine):
                ...
                if index_params.get("quantizer_add_efSearch", 64) > 0:
    ...
    def query(self, X, n):
        ...
        if self._query_bs == -1:
            self.res2 = self.index2.search(X, n)
        else:
